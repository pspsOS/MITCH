...
genericDevice_t MS5607_init(SPI_HandleTypeDef *bus, GPIO_TypeDef *port, uint16_t pin) {
	...
	cmd = 0x1E;
	if(MS5607_send(bmp, &cmd, 1)) return gBMP;

	//Get value of C1
	cmd = 0xA1;
	if(MS5607_receive(bmp, &cmd, 1, dataIn, 2)) return gBMP;
...
HAL_StatusTypeDef MS5607_read(Device_u* device) {
	...
	if (MS5607_send(bmp, &cmd, 1)) return bmp->state;
	HAL_Delay(CONV_T_1024);

	cmd = 0x00;
	if (MS5607_receive(bmp, &cmd, 1, dataIn, 2)) return bmp->state;
